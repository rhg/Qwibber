<%namespace name="base" file="base.mako">
  ...
  <%def name="list(data, cnt)">
    <div id="msg-${cnt}" class="chatItem ${getStateClass(data)}">
          ...
          ${base.sidebar(data)}
					<div class="myBubble">
						...
						<table class="tableBubble" cellspacing="0" cellpadding="0">
							...
							<tr>
								<td class="message">
                  ${base.title(data)} / <span class="text"><a href="${base.listlink(data)}">${data['name']}</a></span><br />
                  <span class="text"><a href="${base.listlink(data)}">${data["full"]}</a>
                  % if data["text"]:
                    <br /><span class="text">${data["text"]}</span>
                  % endif
                </td>
