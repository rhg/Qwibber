...
class GConfValue(object):
   ...
   (or type of data) and GConfValue keeps track of its integrity. It adds the
    ...
    # data
    def get_data(self):
        if hasattr(self, "default") and not self.client.get(self.key):
            return self.default

        return self._getter(self.key)
    
    def set_data(self, value):

        if hasattr(self, "default") and self.default == value and \
            not self.client.get(self.key):
          return
...
create_persistency_link.append_handler(gtk.ToggleAction, _persistency_link_toggle_button)
