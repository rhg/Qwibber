...
class AccountWidget(gtk.VBox):
  """AccountWidget: A widget that provides a user interface for configuring statusnet accounts in Gwibber
  """
  
  def __init__(self, account=None, dialog=None):
    """Creates the account pane for configuring statusnet accounts"""
    gtk.VBox.__init__( self, False, 20 )
    self.ui = gtk.Builder()
    self.ui.set_translation_domain ("gwibber")
    self.ui.add_from_file (resources.get_ui_asset("gwibber-accounts-statusnet.ui"))
    ...
    try:
      if self.account.has_key("access_token") and self.account.has_key("secret_token") and self.account.has_key("username") and has_secret_key:
        ...
        self.ui.get_object("statusnet_auth_done_label").set_label(_("%s has been authorized by %s") % (self.account["username"], self.account["site_display_name"]))
      ...
      if not self.account.has_key("url_prefix") and self.account.has_key("domain"):
        pref = "" if self.account["domain"].startswith("http") else "https://"
        self.url_prefix = self.account["domain"]
        self.url_prefix = pref + self.account["domain"]
        self.ui.get_object("url_prefix").set_text(self.url_prefix)
  ...
  def on_statusnet_auth_clicked(self, widget, data=None):
    self.url_prefix = self.ui.get_object("url_prefix").get_text()
    pref = "" if self.url_prefix.startswith("http") else "https://"
    self.url_prefix = pref + self.url_prefix
    self.ui.get_object("url_prefix").set_text(self.url_prefix)
    ...
    self.ui.get_object("table_common_settings").hide()
  ...
  def on_statusnet_auth_title_change(self, web=None, title=None, data=None):
    ...
    if title.get_title() == "Success":
      try:
        ...
        if isinstance(account_data, dict) and isinstance(site_data, dict):
          if account_data.has_key("screen_name") and site_data.has_key("site"):
            ...
            self.account["url_prefix"] = self.url_prefix
            self.account["site_display_name"] = "%s (%s)" % (site_data["site"]["name"],account_data["screen_name"])
            saved = self.dialog.on_edit_account_save()
          else:
            print "Failed"
        else:
        ...
        self.ui.get_object("hbox_statusnet_auth").hide()
        self.ui.get_object("statusnet_auth_done_label").set_label(_("%s has been authorized by %s") % (str(self.account["username"]), self.account["site_display_name"]))
        self.ui.get_object("hbox_statusnet_auth_done").show()
      ...
      self.ui.get_object("table_common_settings").show()
